...
And it is frictionless where it matters: **you are the client, not the process operator**. [Specs](docs/glossary.md#spec), [briefs](docs/glossary.md#brief), task state, and gate config are all generated by the agents themselves. You hear back for two reasons only: decisions that belong to you, and delivery with evidence.
...
Gates are your own commands in `company/gates.config`; edit them anytime. For real emergencies there is a [hotfix](docs/glossary.md#hotfix) mode that logs instead of blocks, and the process catches up afterward.
