...
_Last updated: 2026-08-22 - delegation doctrine and CR queue SHIPPED (#128, #129). arm-risk-band HALTED by audit, not merged. 0.3.1 prepared but NOT published. v0.3.0 remains registry latest._

## Active tasks

| Task | Class | Lead/Agent | State | Gates | Notes |
|---|---|---|---|---|---|
| arm-risk-band | feature | CEO (self, one seam) + 5 independent audits | **HALTED - not merged.** Branch `task/arm-risk-band`, issue #130 | hooks 772, CLI 62, installer 97, TUI 22, update 139, witnesses 38/38, no-slop clean | Arms DECISIONS #19's compensating control as guard_provenance mode F. Five audits, five HALTs. The first is fully remediated and mutation-tested. The second found the blocker: the RISK subject is the integrand but the EVIDENCE subject is still the local checkout, so an audit of lane A satisfies a merge of lane B (reproduced: unaudited 84-point lane integrated silently, logged as audited). Cannot be patched in place - see the P0 below. Open question OQ-ARB-05 in `company/specs/spec-arm-risk-band.md` is the single thing blocking. RED stays RED. |
| delegation-doctrine | feature | CEO (self) | SHIPPED 2026-08-22 (PR #128 merged b5e9903) | CI 11/11; all five suites green | The harness priced delegation at zero and nagged toward it every turn. Six one-directional prompt pressures rebalanced on the SEAM principle - headcount is a consequence of the work, not a target. `quick` widened to cover internal engineering work. DECISIONS #25. |
| release-readiness-031 | quick | CEO (self) | SHIPPED 2026-08-22 (PR #129 merged 84dd95c) | CI 11/11 | Cleared the CR queue (three CRs decided in code but reading PROPOSED) and re-verified two worry rows whose evidence had gone stale. DECISIONS #26. |
| v0.3.1 release | release | CEO | **PREPARED, NOT PUBLISHED** | R1/R2/R4/R8 green; R3 red (20 orphan IDs, accepted class at v0.2.6); R7 red: two open P0s and two P1s | Registry latest remains 0.3.0. Not published on an unreviewed run of five HALTs with two P0s and two P1s open - DECISIONS #27 records the reasoning so the call is reviewable. |
