...
_Last updated: 2026-07-29 - multi-session-tasks SHIPPED (PR #93 merged 171421f); v0.2.6 prepared and awaiting the owner's tag._

## Active tasks

| Task | Class | Lead/Agent | State | Gates | Notes |
|---|---|---|---|---|---|
| v0.2.6 release | release | CEO | PREPARED 2026-07-29 - AWAITING OWNER TAG | all four suites green on integrated main (393 / 62 / 96 / 139); readiness R2 R4 R8 green, R1 restamped, R3 and R7 red - see the DECISIONS proposal | Carries multi-session-tasks (PR #93) plus the CI coverage fix below. Two readiness criteria are RED and the owner is being asked to accept or hold: R7 (two open P1 worries - the guard_commit worktree row and the provenance verdict-parser row) and R3 (trace_check exits 0 but reports 27 orphan MST requirement IDs with no code or test citation). Neither is a defect in what users install; both are recorded rather than waived. |
