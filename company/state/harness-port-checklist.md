...
| DevMesh deleted the Workflow-tool prohibition from COMPANY | REJECTED | Undocumented, and `models.json` and `METHOD.md` still cite the rule. The premise (internal `agent()` spawns fire no PreToolUse events, so `guard_models` cannot pin them) is empirically checkable and nobody has checked it. Sent back to DevMesh as a finding. |
| DevMesh reverted COMPANY to single-task wording | REJECTED | Contradicts the concurrency layer shipped in the same session. Sent back as a finding. |
