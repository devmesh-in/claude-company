...
// decide every file's fate (keep, replace with backup, land beside as .new,
// or --override: packaged bytes win and retired payload is deleted).
//
...
function helpText() {
  return [
    ...
    "in a project that already ran install. By default it NEVER overwrites a",
    "file you customized: your edit stays and the new upstream version lands",
    "beside it as <file>.new for you to reconcile. The run still exits 0",
    "when that happens. Anything it does replace is backed up first to",
    "company/state/.update-backups/<timestamp>/.",
    "",
    "--override skips that: every shipped file is replaced with the packaged",
    "bytes and retired payload (old skills, ORCHESTRATOR.md) is deleted.",
    "No backups, no .new. Your gates, specs, briefs, and company/state are",
    "still never touched.",
    "",
    ...
    "  --override           Replace shipped files outright. No backups, no .new.",
...
async function run(argv) {
  ...
  if (args.override) engineArgs.push("--override");
