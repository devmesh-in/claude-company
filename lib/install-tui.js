...
function heroRows(text) {
  ...
  for (const ch of text) {
    ...
    const glyph = FONT_RAW[ch.toUpperCase()];
    if (!glyph) continue;
    for (let r = 0; r < 5; r++) {
      rows[r] += glyph[r].replace(/#/g, BLOCK).replace(/\./g, " ") + " ";
    }
  }
...
function drawHero(pal) {
  ...
  out(CLEAR_SCREEN);
  const rows = heroRows("CLAUDE");
  const rows2 = heroRows("COMPANY");
  const lines = ["", ""];
  for (let i = 0; i < rows.length; i++) {
    lines.push(center(pal.paint(rows[i], pal.gradFg(i / 4.0), pal.bold), width));
  }
  lines.push("");
  for (let i = 0; i < rows2.length; i++) {
    lines.push(center(pal.paint(rows2[i], pal.gradFg(0.4 + i / 6.0), pal.bold), width));
  }
  lines.push("");
