...
function runInstallStream(pal, target, activity, harness) {
  return new Promise((resolve) => {
    const env = Object.assign({}, process.env, { PYTHONDONTWRITEBYTECODE: "1" });
    const argv = [INSTALL_SH, harness || "--harness=claude", target];
...
async function screenInstall(pal, target, harness) {
  out(CLEAR_SCREEN);
  const header = [
    "",
  ...
  const [rc, tail] = await runInstallStream(pal, target, activity, harness);
  activity.stop();
  return [rc, tail];
}
...
function runPlain(args) {
  ...
  const res = spawnSync("bash", [INSTALL_SH, harnessArg, p], { env, stdio: "inherit" });
  const rc = res.status;
  if (res.error || rc !== 0) {
