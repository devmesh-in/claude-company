...
function probeOpencode() {
  const v = toolVersion("opencode", ["--version"]);
  return v === null
    ? [WARN, "not found - only needed if you want the opencode harness"]
    : [PASS, v];
}
...
const PROBES = [
  ...
  { name: "opencode CLI", hard: false, fn: probeOpencode },
...
// Appended, never inserted: callers read these positionally
// (chosen.items[0][1]), so inserting a row above an existing one silently
// reassigns what every later index means.
const OPT_DETECT_GATES = 0;
const OPT_ORIENTATION = 1;
const OPT_OPENCODE = 2;

function makeOptions(detectGates, orientation, opencode) {
  return {
    items: [
      ...
      ["Also wire the opencode harness (.opencode/)", opencode],
    ],
  };
}

// The guards and skills install regardless of selection, so this only decides
// whether the opencode adapter and its generated agents and commands come too.
function harnessFlag(opts) {
  return opts && opts.items[OPT_OPENCODE] && opts.items[OPT_OPENCODE][1]
...
const EPILOGUE_PREFIXES = [
  ...
  "/company-init", "/lean-company", "/company", "Next steps:", "Configure",
...
function runInstallStream(pal, target, activity, harness, extraArgs) {
  return new Promise((resolve) => {
    const env = Object.assign({}, process.env, { PYTHONDONTWRITEBYTECODE: "1" });
    const argv = [INSTALL_SH, harness || "--harness=claude"]
      .concat(extraArgs || [])
      .concat([target]);
    const proc = spawn("bash", argv, { env, stdio: ["ignore", "pipe", "pipe"] });
    const tail = [];
...
async function screenInstall(pal, target, harness, extraArgs) {
  out(CLEAR_SCREEN);
  const header = [
    "",
  ...
  const [rc, tail] = await runInstallStream(pal, target, activity, harness, extraArgs);
  activity.stop();
  return [rc, tail];
}
...
function screenDone(pal, target, orientation) {
  ...
  const orch = pal.paint("/company build me <what you want>", pal.gradFg(0.5), pal.bold);
  if (orientation) {
...
async function runTui(args, pal) {
  ...
  try {
    ...
    // Pre-checked, not auto-selected: detection sets the default and the
    // person still confirms it on the Options screen.
    const opencodeDefault =
      args.harness.indexOf("opencode") !== -1 ||
      probeOpencode()[0] === PASS ||
      fs.existsSync(path.join(target, ".opencode"));
    const opts = makeOptions(args.detectGates, args.orientation, opencodeDefault);
    ...
    const [rc, tail] = await screenInstall(pal, target, harnessFlag(chosen),
      args.noBgEnv ? ["--no-background-subagents-env"] : []);
    if (rc !== 0) {
...
function runPlain(args) {
  ...
  process.stdout.write("\ninstalling...\n");
  const env = Object.assign({}, process.env, { PYTHONDONTWRITEBYTECODE: "1" });
  // Non-interactive: whatever --harness said, defaulting to claude. There is
  // no detection here on purpose - an unattended install must be reproducible.
  const harnessArg = "--harness=" + args.harness;
  const extra = args.noBgEnv ? ["--no-background-subagents-env"] : [];
  const res = spawnSync("bash", [INSTALL_SH, harnessArg].concat(extra).concat([p]), { env, stdio: "inherit" });
  const rc = res.status;
  if (res.error || rc !== 0) {
  ...
  process.stdout.write("  next: open Claude Code and run  /company build me <what you want>\n");
  return EXIT_OK;
}
...
function helpText() {
  return [
    ...
    "                       Skip writing the background-subagents env export",
    "                       into your shell profile (opencode harness only).",
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    } else if (a === "--no-background-subagents-env") {
      args.noBgEnv = true;
    } else if (a.startsWith("--harness=")) {
      args.harness = a.slice("--harness=".length);
    } else if (a === "--target") {
      args.target = argv[++i];
